...
BUNDLE_ID   := jp.nlink.active-lens-gui
VERSION     := $(shell git describe --tags --always --dirty 2>/dev/null || echo "0.1.0")
BUILD_DIR   := .build/release
DIST_DIR    := dist
APP_BUNDLE  := $(DIST_DIR)/$(APP_NAME).app

# The active-lens CLI is the data + sampling backend. build-app bundles it into
...
# deployment target instead of the SDK actually used (measured: sdk 14.0, where
# every app built here before the update records the real SDK), so a release
...
build-app: build
	...
	@if [ -x "$(CLI_BIN)" ]; then \
		cp "$(CLI_BIN)" $(APP_BUNDLE)/Contents/Resources/active-lens; \
		echo "[bundle] embedded CLI from $(CLI_BIN)"; \
	else \
		echo "[bundle] WARN: CLI binary $(CLI_BIN) not found — app will locate it via PATH / \$$ACTIVE_LENS_BIN at runtime"; \
	fi
	@if [ -f "$(ICON_SRC)" ]; then \
...
verify-release:
	...
	@sdk=$$(otool -l "$(APP_BUNDLE)/Contents/MacOS/$(APP_NAME)" | awk '/LC_BUILD_VERSION/{f=1} f && /^ *sdk /{print $$2; exit}'); \
		test "$$sdk" = "$(MACOS_SDK)" || { \
			...
			exit 1; }
	@cli="$(APP_BUNDLE)/Contents/Resources/active-lens"; \
