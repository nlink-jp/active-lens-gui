...
final class SingleInstanceTests: XCTestCase {
    ...
    func testNoRunningInstancesProceeds() {
        XCTAssertEqual(
            singleInstanceDecision(
                bundleID: "jp.nlink.active-lens-gui", ownPID: 42, instancePIDs: []
            ),
            .proceed
        )
    }
    ...
    func testOwnPIDAloneProceeds() {
        ...
        XCTAssertEqual(
            singleInstanceDecision(
                bundleID: "jp.nlink.active-lens-gui", ownPID: 42, instancePIDs: [42]
            ),
            .proceed
        )
    }
    ...
    func testAnotherInstanceExits() {
        guard case .exitDuplicate(let message) = singleInstanceDecision(
            bundleID: "jp.nlink.active-lens-gui", ownPID: 42, instancePIDs: [97316]
        ) else {
    ...
    func testAllOtherPIDsAreListed() {
        guard case .exitDuplicate(let message) = singleInstanceDecision(
            bundleID: "jp.nlink.active-lens-gui", ownPID: 1, instancePIDs: [1, 2, 3]
        ) else {
            return XCTFail("expected exitDuplicate")
        }
